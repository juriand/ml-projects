...
for i in range(10):
    X_train, X_test, y_train, y_test = train_test_split(Xs[i], ys[i], test_size=1.0/3, random_state=int("110"))
    for d in range(15):
        dt = DecisionTreeClassifier(max_depth=d+1, random_state=int("039"))
        dt = dt.fit(X_train, y_train)

        train_ac[i][d] = accuracy_score(y_train, dt.predict(X_train))
        test_ac[i][d] = accuracy_score(y_test, dt.predict(X_test))
